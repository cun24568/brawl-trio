...
def _startup():
    ...
    # キュー計測ポーリング worker
    _t.Thread(target=_queue_polling_loop, daemon=True).start()
...
# ============================================================
QUEUE_POLL_INTERVAL = 12        # 各タグの battlelog ポーリング間隔(秒)
QUEUE_TIMEOUT_SEC = 30 * 60     # 30分以上マッチしなければ timeout
QUEUE_LOOP_TICK = 5             # メインループ tick (秒)
# 公式APIの rate limit ((Bearer key の制限ベース) を考慮した最大同時計測数
QUEUE_MAX_CONCURRENT = 8

# in-memory state: {measurement_id: last_polled_at}
_queue_last_polled: dict[int, float] = {}
_queue_baseline_battletime: dict[int, str] = {}  # キュー開始時点の最新battleTime (これより新しいのが入ったら matched)
_queue_state_lock = threading.Lock()


def _queue_polling_loop():
    """常駐worker: pending な queue_measurements を巡回ポーリング。"""
    import traceback
    while True:
        try:
            pendings = db.list_pending_queue_measurements(max_age_seconds=QUEUE_TIMEOUT_SEC + 60)
            now = time.time()
            # timeout 判定
            for m in pendings:
                if int(now) - m["queue_start_at"] > QUEUE_TIMEOUT_SEC:
                    db.mark_queue_timeout(m["id"])
                    with _queue_state_lock:
                        _queue_last_polled.pop(m["id"], None)
                        _queue_baseline_battletime.pop(m["id"], None)
            pendings = [m for m in pendings if int(now) - m["queue_start_at"] <= QUEUE_TIMEOUT_SEC]
            # 同時計測上限
            pendings = pendings[:QUEUE_MAX_CONCURRENT]
            for m in pendings:
                mid = m["id"]
                # ポーリング間隔チェック
                with _queue_state_lock:
                    last = _queue_last_polled.get(mid, 0)
                if now - last < QUEUE_POLL_INTERVAL:
                    continue
                _poll_one_queue(m)
                with _queue_state_lock:
                    _queue_last_polled[mid] = now
        except Exception:
            traceback.print_exc()
        time.sleep(QUEUE_LOOP_TICK)


def _poll_one_queue(m: dict):
    """1つの measurement に対し battlelog を取得 → 新試合検出"""
    mid = m["id"]
    tag = m["tag"]
    try:
        battles = brawl_api.get_battlelog(tag)
    except HTTPError as e:
        # 429 などは一旦スキップ (次回 tick で再試行)
        if e.code in (404,):
            db.mark_queue_timeout(mid)
        return
    except Exception:
        return
    if not battles:
        return
    # baseline 未設定なら最新battleTime を baseline に
    with _queue_state_lock:
        baseline = _queue_baseline_battletime.get(mid)
    latest_bt = max((b.get("battleTime", "") for b in battles), default="")
    if baseline is None:
        with _queue_state_lock:
            _queue_baseline_battletime[mid] = latest_bt
        return
    # baseline より新しい battle を探す
    new_battles = [b for b in battles if b.get("battleTime", "") > baseline]
    if not new_battles:
        return
    # 試合終了時刻が queue_start_at よりも後で、かつ duration を引いた開始時刻が queue_start_at より後のものだけ採用
    qs = m["queue_start_at"]
    candidates = []
    for b in new_battles:
        bt = b.get("battleTime", "")
        end_unix = db._parse_battle_time_unix(bt)
        if end_unix is None:
            continue
        battle_inner = b.get("battle") or {}
        duration = battle_inner.get("duration", 0) or 0
        start_unix = end_unix - duration
        # 試合開始が queue_start_at より過去のものは無視 (キュー押す前に始まってる)
        if start_unix < qs - 5:  # 5秒の誤差許容
            continue
        candidates.append((start_unix, b))
    if not candidates:
        return
    # 一番早く始まった試合を採用
    candidates.sort(key=lambda x: x[0])
    chosen = candidates[0][1]
    ok = db.complete_queue_measurement(mid, chosen)
    if ok:
        with _queue_state_lock:
            _queue_last_polled.pop(mid, None)
            _queue_baseline_battletime.pop(mid, None)
        # 試合データも battles テーブルに保存しておく (ついでに)
        try:
            db.upsert_battles(tag, [chosen])
        except Exception:
            pass
...
def queue_start(request: Request, tag: str):
    """キュー開始ボタン: queue_measurements に pending 行を追加。 measurement_id を返す。"""
    tag = _normalize_or_400(tag)
    # 同時計測上限
    pendings = db.list_pending_queue_measurements(max_age_seconds=QUEUE_TIMEOUT_SEC + 60)
    if len(pendings) >= QUEUE_MAX_CONCURRENT and not any(p["tag"] == tag for p in pendings):
        raise HTTPException(
            status_code=429,
            detail=f"queue measurement is full ({QUEUE_MAX_CONCURRENT} concurrent). retry later.",
        )
    mid = db.create_queue_measurement(tag)
    # 即座に1回ポーリングしてbaseline 確定 (試合開始判定のため)
    threading.Thread(
        target=_poll_one_queue,
        args=({"id": mid, "tag": tag, "queue_start_at": int(time.time())},),
        daemon=True,
    ).start()
    return {
        "measurement_id": mid,
        "tag": tag,
        "queue_start_at": int(time.time()),
        "poll_interval_seconds": QUEUE_POLL_INTERVAL,
        "timeout_seconds": QUEUE_TIMEOUT_SEC,
    }


@app.get("/api/queue/{measurement_id}")
@limiter.limit("60/minute")
def queue_get(measurement_id: int, request: Request):
    """計測結果を取得。 status: pending / matched / timeout / canceled。"""
    m = db.get_queue_measurement(measurement_id)
    if not m:
        raise HTTPException(status_code=404, detail="measurement not found")
    return m


@app.get("/api/queue/recent/{tag}")
@limiter.limit("30/minute")
def queue_recent(tag: str, request: Request, limit: int = 30):
    """そのタグの過去計測一覧。"""
    tag = _normalize_or_400(tag)
    if limit > 100:
        limit = 100
    rows = db.recent_queue_measurements(tag=tag, limit=limit)
